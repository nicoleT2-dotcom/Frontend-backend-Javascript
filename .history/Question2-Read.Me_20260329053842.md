...
**`split()`** is a method that works on strings by spliting the string into parts. This method doesn't change the original string. The `split()` method then creates a new array where each part then becomes an item in the array. 
