...
**`split()`** is a method that works on strings by spliting the string into parts. This method doesn't change the original string. The `split()` then creates a new array where each part then becomes a item in the array. 
...
**`split()`** splits the string in to parts where ever there are spaces. 
