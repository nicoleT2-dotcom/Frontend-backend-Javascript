...
**`onmouseover`** is an event that is triggered whenever the mouse/pointer hovers over an element on a webpage.

**`onclcick`** is an event that is triggered whenever a the specific button it's attached to is clicked 

**`touchmove`** is an event that is triggered whenever 
